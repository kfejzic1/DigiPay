...
namespace AdministrationAPI.Services
{
    public class ExchangeService : IExchangeService
    {
        ...
        const string PROCESSING_SERVER_URL = "https://localhost:7146";
        ...
        public async Task<(string message,CurrencyAccount?obj)> CreateAccount(ExchangeAccountRequest request, string token)
        {
            try
            {
                ...
                var response = await client.PostAsync(PROCESSING_SERVER_URL + "/api/UserBankAccount/CreateAccount?token=" + token.Substring(7),
                    content1);
        ...
        public async Task<(string message,List<CurrencyAccount>? obj)> GetUserAccounts(string token)
        {
            try
            {
                ...
                if (token.Length < 8)
                    return ("Invalid token!", null);
                var response = await client.GetAsync(PROCESSING_SERVER_URL + "/api/UserBankAccount/GetAllAccountsForUser?token=" + token.Substring(7));
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
        ...
        public async Task<(string message, List<CurrencyAccount>? obj)> GetAllAccounts(string token)
        {
            try
            {
                ...
                if (token.Length < 8)
                    return ("Invalid token!", null);
                var response = await client.GetAsync(PROCESSING_SERVER_URL + "/api/UserBankAccount/GetAllAccounts?token=" + token.Substring(7));
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    var userTransfer = await response.Content.ReadFromJsonAsync<List<CurrencyAccount>>();
                    return ("", userTransfer);
        ...
        public async Task<(string message,TransactionResponse? obj)> MakeTransaction(TransactionRequest transactionRequest, string token)
        {
            try
            {
                ...
                var response = await client.PostAsync(PROCESSING_SERVER_URL + "/api/Transaction/CreateTransaction?token=" + token.Substring(7),
                    content1);
                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
